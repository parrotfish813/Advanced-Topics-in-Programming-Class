/* PLEASE READ THIS */
/* This file is designed so that you can just copy and paste the whole thing into MySQL and have it Just 
Work. HOWEVER, That’s not a good excuse to not check and see what I changed! I left green comments 
on all my edits. */
/* Save and run the entire script in one file */
/* Student information database tables 
   Run this from the root */
drop database if exists javabook; /* Makes sure javabook doesn’t exist before trying to remove it. */
create database javabook;
create user if not exists 'scott'@'localhost' identified by 'tiger'; /* Makes sure scott isn’t a user before trying 
to add him. */
...
insert into Faculty values ( 
  111221111, 'George', 'P', 'Frank1lin', '9129212525', 
...
insert into Faculty values ( 
  '111221113', 'Frank1', 'E', 'Goldman', '9129215557',
  'frank1@yahoo.com', 'SC132', '1996-01-14', 
...
insert into Student values (
  '444111113', 'Frank1', 'E', 'Jones', '9125919434',
...
insert into Student values (
  ...
  '1970-02-09', '555 Frank1lin Street', '31411', 'CHEM');
insert into Student values (
...
insert into StateCapital values (
  'Arizona', 'Phoenix');
*/
drop table if exists Country; /* Makes sure Country doesn’t exist before trying to drop it. */
...
/* Moved the following section from lower to avoid a “commands out of sync” error relating, I believe, to 
the “studentFound” function’s delimiter changes. This error can also be avoided by running this chunk and 
the studentFound function’s create statement one at a time, but I wanted the whole file to be runnable as-
is. */
...
create function studentFound(first VARCHAR(20), last VARCHAR(20))
  returns int
  READS SQL DATA    /* This tells MySQL that this function doesn’t edit any data, just reads it. If you 
don’t want to include this or any of its variants (NO SQL, DETERMINISTIC, MODIFIES SQL DATA, or 
CONTAINS SQL), you can run “ SET GLOBAL log_bin_trust_function_creators = 1; ” This will make SQL 
just trust you, however, you will have to enter your admin password as it is a change to a global variable. 
*/
begin
  declare result int;
